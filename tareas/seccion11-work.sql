...
select count(*) as total_post_publicados from posts where published is true;
...
select count(*), created_by, b.username from posts
inner join users b on b.user_id=posts.created_by
GROUP by created_by, b.username order by COUNT(*) desc limit 10;
...
select sum(counter) as total, posts.title from claps
inner join posts on posts.post_id=claps.post_id

GROUP by  posts.title
order by sum(counter) desc limit 5
;
...
364	Lillian Hodge
*/
select sum(counter), name from claps
inner JOIN users ON users.user_id = claps.user_id
group by name
...
left join user_lists on users.user_id=user_lists.user_id where user_list_id is null
;
