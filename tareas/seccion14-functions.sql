...
-- 2do problema calcular la diferencia entre el salario maximo y el salario actual
...
--funcion que calcula la diferencia entre el salario maximo y el salario actual
create or replace function max_raise(empl_id int)

returns int numeric(8,2) as
...
begin
    select 
        max_salary-salary as salary_difference into possible_raise
from employees
inner join jobs on jobs.job_id=employees.job_id
